...
namespace DocumentVault.Services.CosmosDbService
{
    public class CosmosDbService : ICosmosDbService
    {
        ...
        {
            var response = await _container.CreateItemAsync(downloadLink, new PartitionKey(downloadLink.partitionKey));
            return response.Resource;
        }
        ...
        public async Task<bool> DeleteDocumentAsync(string id)
        {
            try
            {
                await _container.DeleteItemAsync<Document>(id, new PartitionKey("documents"));
                return true;
            }
        ...
        public async Task<bool> DeleteDownloadLinkAsync(string linkId)
        {
            try
            {
                await _container.DeleteItemAsync<DownloadLink>(linkId, new PartitionKey("links"));
                return true;
            }
            catch (CosmosException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
        ...
        public async Task<IEnumerable<Document>> GetAllDocumentsAsync()
        {
            ...
            var queryIterator = _container.GetItemQueryIterator<Document>(queryDefinition);
        ...
        public async Task<Document?> GetDocumentByIdAsync(string id)
        {
            try
            {
                var response = await _container.ReadItemAsync<Document>(id, new PartitionKey("documents"));
                return response.Resource;
            }
            catch (CosmosException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
        ...
        public async Task<DownloadLink?> GetDownloadLinkAsync(string linkId)
        {
            try
            {
                var response = await _container.ReadItemAsync<DownloadLink>(linkId, new PartitionKey("links"));
                return response.Resource;
            }
            catch (CosmosException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
        ...
        public async Task<bool> UpdateDownloadLinkAsync(DownloadLink downloadLink)
        {
            try
            {
                // Fixed: Use downloadLink.PartitionKey instead of downloadLink.id for consistency
                await _container.ReplaceItemAsync(downloadLink, downloadLink.id, new PartitionKey(downloadLink.partitionKey));
                return true;
            }
            catch (CosmosException ex) when (ex.StatusCode == System.Net.HttpStatusCode.NotFound)
            {
